...
if (isset($_POST['send']))
    {
        ...
        $insert="INSERT INTO `add_stock`(`Designation`, `Reference`, `Departement`, `Qte`, `Unite`, `Qte_totale`, `Prix_unitaire`, `Prix_total`, `datee`)
        VALUES ( '$Designation', '$Reference', '$Departement', '$Qte', '$Unite','$Qte_totale','$Prix_unitaire', '$Prix_total', NOW())";
...
?>
  ...
  <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
	...
	<h5>Veiller Ajouter dans le stock</h5>

	<form method="POST" action="Entrer_item.php" enctype="multipart/form-data">
      ...
      <div class="table-responsive "style="display:block; height:400px; overflow: auto;" >
        ...
        <thead class="thead-dark">
          ...
          <!-- style="display:block; height:200px; overflow: auto;" -->
            ...
            <?php
                include('configuration/dbConnection.php ');
                 $select="select * from add_stock ORDER BY id DESC";
                $run_query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_assoc($run_query))
                {
            ?>
          ...
          <?php
                include('configuration/dbConnection.php');
                $select="select COUNT(*) AS total from add_stock";
                $run_query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_assoc($run_query))
                {
            ?>
