...
if (isset($_POST['add']))
    {
        extract($_POST);
        $insert="INSERT INTO `lieu_affectation`(`lieu`, `project`, `datee`)
        VALUES ( '$lieu', '$project',NOW() )";
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Mont Gabaon Lieu d'affectaction</h1>
	...
	<h5>Veiller Ajouter un lieu d'Affectation</h5>
	<form method="POST" action="lieu_daffectation.php" enctype="multipart/form-data">
        <div class="row mb-3 mt-3">
            <div class="col-md-3 col-sm-6">
            <label>Lieu d'affectation</label>
            <input type="text" class="form-control" placeholder="Entrez le lieu du travail" name="lieu" required>
    </div>

    <div class="col-md-3 col-sm-6">
        <label>Le nom du project</label>
        <input type="text" class="form-control" placeholder="Entrez le nom du project" name="project"required >
    </div>
    </div>
...
<!-- style="display:block; height:200px; overflow: auto;" -->
            ...
            <?php
                include('configuration/dbConnection.php');
                 $select="select * from lieu_affectation ORDER BY id DESC";
            ...
            <?php
                include('configuration/dbConnection.php');
                $select="select COUNT(*) AS total from lieu_affectation";
                $run_query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_assoc($run_query))
                {
            ?>
